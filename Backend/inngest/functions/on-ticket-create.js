...
import {
  emailNotificationForTicketMailGenContent,
  sendMail,
  ...
  async ({ event, step }) => {
    ...
    const relatedSkills = await step.run("ai-processing", async () => {
      ...
      if (aiResponse) {
        ...
        await Ticket.findByIdAndUpdate(ticketID, {relatedSkills: skills})
        return skills;
      }
    });
    // assigning mods
    ...
    await step.run("send-notification-to-mods", async () => {
      ...
      const content = emailNotificationForTicketMailGenContent(
        mod.name,
        finalTicket,
      );
      const options = {
